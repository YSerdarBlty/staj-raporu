...
def get_ticket_counts():
    if current_user.is_admin:
        ...
        pending_count = Ticket.query.filter_by(status='in_progress').count()
    else:
        ...
        pending_count = Ticket.query.filter_by(user_id=current_user.id, status='in_progress').count()
